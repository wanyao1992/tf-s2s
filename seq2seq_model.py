...
class Seq2SeqModel(object):
    def __init__(self, args, dicts):
        self.args = args
        self.mode = args.mode
        assert self.mode.lower() in ['train', 'test']
        ...
        self.num_encoder_symbols = dicts['src'].size()
        self.num_decoder_symbols = dicts['tgt'].size()

        self.use_residual = args.use_residual
        self.attn_input_feeding = args.attn_input_feeding
        self.use_dropout = args.use_dropout
        self.keep_prob = 1.0 - args.dropout_rate
        ...
        self.dtype = tf.float32 #tf.float16 if args.use_fp16 else tf.float32
    ...
    def build_model(self):
        ...
        if self.mode == 'train':
            ...
            decoder_start_token = tf.ones(
                shape=[self.batch_size, 1], dtype=tf.int32) * Constants.BOS # data_utils.start_token
            decoder_end_token = tf.ones(
                shape=[self.batch_size, 1], dtype=tf.int32) * Constants.EOS # data_utils.end_token
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.mode == 'train':
                ...
                start_tokens = tf.ones([self.batch_size, ], tf.int32) * Constants.BOS # data_utils.start_token
                end_token = Constants.EOS # data_utils.end_token
